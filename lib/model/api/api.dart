...
class Pushes {
  ...
  Future<void> savePushToFile(String mode) async {
    ...
    if (Platform.isAndroid) {
      if (mode == 'onMessage') {
        _toSave['id'] =
            parsePushForId(lastMessage['notification']['title'].toString());
        _toSave['title'] = lastMessage['notification']['title']
            .toString()
            .substring(
                lastMessage['notification']['title'].toString().indexOf(')') +
                    2);
        ...
        _toSave['title'] = lastMessage['data']['title'].toString().substring(
            lastMessage['data']['title'].toString().indexOf(')') + 2);
...
class RestAPI {
  ...
  Future<String> userDataGet(String guid, String token) async {
    ...
    try {
      response = await get(url, headers: headers).timeout(Duration(seconds: 5),
          onTimeout: () {
