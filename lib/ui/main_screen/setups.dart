...
class SetupGroupWidget extends State {
                                              ...
                                              onTap: () => Navigator.of(context)
                                                  .push(MaterialPageRoute(
                                                      builder: (BuildContext
                                                              context) =>
                                                          PayView())), // Харитон, тут надо тапнуть на новый скрин пополнения баланса
                                              iconData: MaterialCommunityIcons
                                                  .wallet_plus_outline),
                                        ),
                                        Text(
                                          NumberFormat('#,##0.00##', 'ru_RU')
                                                  .format(double.parse(userInfo[
                                                      "extra_account"])) +
                                              " р.",
                                          //userInfo["extra_account"],
                                          style: TextStyle(
                                            color: double.parse(userInfo[
                                                        "extra_account"]) <
                                                    0
                                                ? Color.fromRGBO(
                                                    255, 81, 105, 1)
                                  ...
                                  Slider(
                                    ...
                                    onChanged: (double days) => setState(() {
                                      ...
                                      if ((days % 10).round() >= 5)
                                        daysText = 'дней';
                                      if (days.round() >= 11 &&
                                          days.round() <= 14) daysText = 'дней';
                                    }),
                                    min: 1,
                                    max: users[currentGuidIndex]['max_days'] >=
                                            1
                                        ? users[currentGuidIndex]['max_days']
                                            .toDouble()
                                        : 1,
                                    divisions: users[currentGuidIndex]
                                        ['max_days'],
  ...
  void onAddDaysButtonPressed() async {
    String answer = await RestAPI()
        .addDaysPUT(_currentDays.round(), guids[currentGuidIndex], devKey);
  ...
  void onSetupAutoChange(value) async {
    String answer = await RestAPI()
        .switchChangePUT('activation', guids[currentGuidIndex], devKey);
    setState(() {
      if (answer.startsWith('is')) {
  ...
  void onSetupParentChange(bool value) async {
    String answer = await RestAPI()
        .switchChangePUT('parent', guids[currentGuidIndex], devKey);
    setState(() {
      if (answer.startsWith('is')) {
